...
class Para_RefGov_SESBOPTES_MW(Validator):
  ...
  def __init__(self):
    ...
    self._unitInfo = { # a two-layer dictionary, containing the matrix, lower/upper constraints, and XLast to start with
      'BOP':{
        'MatrixFile':"D:\\GitProjects\\wanghy_fork\\HERON\\tests\\integration_tests\\validator_FARM\\BOP_SES_TES_Mat_Unit_MW\\DMDcCxCoeff_BOP.xml",
      ...
      'SES':{
        'MatrixFile':"D:\\GitProjects\\wanghy_fork\\HERON\\tests\\integration_tests\\validator_FARM\\BOP_SES_TES_Mat_Unit_MW\\DMDcCxCoeff_SES.xml",
      ...
      'TES':{
        ...
        'MatrixFile':"D:\\GitProjects\\wanghy_fork\\HERON\\tests\\integration_tests\\validator_FARM\\BOP_SES_TES_Mat_Unit_MW\\DMDcCxCoeff_TES2.xml",
    ...
    for comp, info in dispatch._resources.items(): 
      ...
      for unit in self._unitInfo: 
        ...
        if str(unit) not in str(comp):
          ...
          Tss, n, m, p, power_array, UNorm_list, XNorm_list, XLast_list, YNorm_list, A_list, B_list, C_list, eig_A_array = read_parameterized_XML(MatrixFile)
          ...
          power_array, UNorm_list, XNorm_list, XLast_list, YNorm_list, A_list, B_list, C_list, eig_A_array = check_YNorm_within_Range(
            y_min, y_max, power_array, UNorm_list, XNorm_list, XLast_list, YNorm_list, A_list, B_list, C_list, eig_A_array)
          if YNorm_list == []:
          ...
          for res in info:
            if str(res) == "electricity":
              ...
              for tidx, time in enumerate(times):
                ...
                profile_id = (np.abs(power_array - r_value)).argmin()
